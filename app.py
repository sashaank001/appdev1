...
# def admin_login():
    ...
    #     if admin_username == 'sashaankadmin' and admin_password == 'sashaank':  
...
def admin_dashboard():
    if not session.get('admin_logged_in'):  # Check if session exists
...
def create_subject():
    if not session.get('admin_logged_in'):  # Check if session exists
        flash('Please log in to access futhur', 'danger')
        return redirect(url_for('admin_login'))
...
def edit_quiz(quiz_id):
    ...
    quiz = Quiz.query.get_or_404(quiz_id)
    if request.method == 'POST':
        # Convert string date input to a Python date object
...
def profile():
    ...
    if not user_id:  # If user_id is None, redirect to login
        return redirect(url_for("index"))

    user = User.query.get(user_id)  # Fetch user from database
    if not user:  # If user does not exist, redirect to login
        return redirect(url_for("index"))
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        dob_str = request.form.get("dob")
        if dob_str:
            user.dob = datetime.strptime(dob_str, "%Y-%m-%d").date()  # Convert string to date
